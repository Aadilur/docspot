...
export type ObjectGroupDetails = {
  group: ObjectGroup;
...
function rowToGroup(row: any): ObjectGroup {
  return {
    id: String(row.id),
...
function rowToReport(row: any): ObjectReport {
  return {
    id: String(row.id),
...
function rowToAttachment(row: any): ObjectAttachment {
...
export async function createGroupWithFirstReport(params: {
  ...
  return await withPostgresTransaction(async (client) => {
    const gRes = await client.query(
      `insert into object_groups (id, user_id, title, created_at, updated_at)
    ...
    const rRes = await client.query(
      `insert into object_reports (id, group_id, user_id, title, issue_date, next_appointment, doctor, text_note, created_at, updated_at)
...
> {
  ...
  const res = await pg.query(
    `select g.*,
            (select count(*)::int from object_reports r where r.group_id = g.id) as report_count,
            (select jsonb_build_object('id', r2.id, 'title', r2.title, 'updatedAt', r2.updated_at)
             from object_reports r2 where r2.group_id = g.id order by r2.created_at desc limit 1) as latest_report
...
export async function getGroupDetails(params: {
  userId: string;
  groupId: string;
}): Promise<ObjectGroupDetails | null> {
  await ensureSchema();
  const pg = getPostgresPool();

  const gRes = await pg.query(
    `select * from object_groups where id = $1::uuid and user_id = $2::uuid`,
    [params.groupId, params.userId],
  );
  if (gRes.rows.length === 0) return null;

  const reportsRes = await pg.query(
    `select * from object_reports
  ...
  const attRes = reportIds.length
    ? await pg.query(
        `select * from object_attachments where report_id = any($1::uuid[]) order by created_at desc`,
        [reportIds],
      )
    : { rows: [] };

  const attsByReport = new Map<string, ObjectAttachment[]>();
...
export async function patchGroup(params: {
  ...
  const res = await pg.query(
    `update object_groups set title = $1::text, updated_at = now()
...
export async function createReport(params: {
  ...
  const gRes = await pg.query(
    `select id from object_groups where id = $1::uuid and user_id = $2::uuid`,
    [params.groupId, params.userId],
  );
  ...
  await pg.query(
    `update object_groups set updated_at = now() where id = $1::uuid and user_id = $2::uuid`,
    [params.groupId, params.userId],
  );
...
export async function patchReport(params: {
  ...
  const sql = `update object_reports
...
export async function addAttachment(params: {
  ...
  const rRes = await pg.query(
    `select id from object_reports where id = $1::uuid and group_id = $2::uuid and user_id = $3::uuid`,
  ...
  const existsRes = await pg.query(
    `select * from object_attachments
  ...
  const res = await pg.query(
    `insert into object_attachments (id, report_id, user_id, key, filename, content_type, size_bytes, kind, created_at, updated_at)
  ...
  await pg.query(
    `update object_reports set updated_at = now() where id = $1::uuid and user_id = $2::uuid`,
    [params.reportId, params.userId],
  );
  await pg.query(
    `update object_groups set updated_at = now() where id = $1::uuid and user_id = $2::uuid`,
...
export async function deleteAttachment(params: {
  ...
  return await withPostgresTransaction(async (client) => {
    const keyRes = await client.query(
      `select a.key as key
       from object_attachments a
       join object_reports r on r.id = a.report_id
       join object_groups g on g.id = r.group_id
       where a.id = $1::uuid
    ...
    await client.query(
      `delete from object_attachments where id = $1::uuid and user_id = $2::uuid`,
...
export async function deleteGroupRow(params: {
  ...
  const delRes = await pg.query(
    `delete from object_groups where id = $1::uuid and user_id = $2::uuid`,
    [params.groupId, params.userId],
  );
...
export async function createShareLink(params: {
  ...
  const res = await pg.query(
    `insert into object_share_links (token, group_id, created_by_user_id, expires_at, created_at)
...
export async function getSharedGroupByToken(params: {
  ...
  const sRes = await pg.query(
    `select token, group_id, expires_at from object_share_links
  ...
  const gRes = await pg.query(
    `select * from object_groups where id = $1::uuid`,
    [groupId],
  );
  if (gRes.rows.length === 0) return null;

  const reportsRes = await pg.query(
    `select * from object_reports where group_id = $1::uuid order by created_at desc`,
    [groupId],
  );
