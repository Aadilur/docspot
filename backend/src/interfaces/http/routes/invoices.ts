...
import { deleteObjects } from "../../../infrastructure/storage/s3";
import { createPresignedGetUrl } from "../../../infrastructure/storage/s3";
import * as invoicesDb from "../../../infrastructure/db/invoices";

type EnsureMe = (req: any) => Promise<{ id: string }>;

export function registerInvoiceRoutes(params: {
  router: Router;
  ...
  router.get("/me/invoice-groups", requireFirebaseAuth, async (req, res) => {
    try {
      ...
      const groups = await invoicesDb.listGroups({
        userId: me.id,
        limit,
        offset,
      });
  ...
  router.post("/me/invoice-groups", requireFirebaseAuth, async (req, res) => {
    ...
    try {
      const me = await ensureMe(req);
      const created = await invoicesDb.createGroupWithFirstReport({
        userId: me.id,
        report: {
          title,
          issueDate:
  ...
  router.get(
    ...
    async (req, res) => {
      try {
        const me = await ensureMe(req);
        const details = await invoicesDb.getGroupDetails({
          userId: me.id,
  ...
  router.patch(
    ...
    async (req, res) => {
      ...
      try {
        const me = await ensureMe(req);
        const updated = await invoicesDb.patchGroup({
          userId: me.id,
          groupId: req.params.id,
          title,
        });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      try {
        const me = await ensureMe(req);
        const report = await invoicesDb.createReport({
          userId: me.id,
          groupId: req.params.id,
          title,
          issueDate:
  ...
  router.patch(
    ...
    async (req, res) => {
      try {
        const me = await ensureMe(req);
        const updated = await invoicesDb.patchReport({
          userId: me.id,
          groupId: req.params.id,
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      try {
        ...
        const attachment = await invoicesDb.addAttachment({
          userId: me.id,
          groupId: req.params.id,
          reportId: req.params.reportId,
          key,
  ...
  router.delete(
    ...
    async (req, res) => {
      try {
        const me = await ensureMe(req);
        const keys = await invoicesDb.getGroupAttachmentKeys({
          userId: me.id,
          groupId: req.params.id,
        });
        ...
        const deleted = await invoicesDb.deleteGroupRow({
          userId: me.id,
          groupId: req.params.id,
        });
        if (!deleted) {
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      try {
        const me = await ensureMe(req);
        const link = await invoicesDb.createShareLink({
          userId: me.id,
          groupId: req.params.id,
          ttlSeconds,
        ...
        res.json({
          ...
          sharePath: `/share/invoices/${link.token}`,
  ...
  router.get("/share/invoices/:token", async (req, res) => {
    ...
    try {
      const shared = await invoicesDb.getSharedGroupByToken({ token });
      if (!shared) {
        notFound(res);
        return;
      }
