...
function bestTitle(group: InvoiceGroup, reports: ReportWithAttachments[]) {
  if (reports.length <= 1) return reports[0]?.title || group.title || "";
  return group.title || reports[0]?.title || "";
}

export default function InvoiceGroupDetailsPage() {
  ...
  async function refresh() {
    ...
    setLoading(true);
    try {
      const details = await getInvoiceGroupDetails(groupId);
  ...
  async function saveGroupTitle() {
    ...
    setError(null);
    try {
      const updated = await patchInvoiceGroup({
        groupId,
  ...
  async function onDeleteGroup() {
    ...
    setBusyGroup(true);
    try {
      await deleteInvoiceGroup(groupId);
      navigate("/invoice");
    } catch (e) {
      setError(e instanceof Error ? e.message : String(e));
    } finally {
  ...
  async function startShareCreate() {
    ...
    setShareResult(null);
    try {
      const res = await createInvoiceShareLink({
        groupId,
        ttlSeconds: ttl,
      });
  ...
  const pageTitle =
    group && reports.length ? bestTitle(group, reports) : t("invoiceDetails");
...
function ReportSheet({
  ...
  async function saveReport() {
    ...
    try {
      ...
      if (!initial) {
        const created = await createInvoiceReport({
          groupId,
          ...patch,
        });
        reportId = created.id;
