...
import {
  ...
  getPrescriptionGroupDetails,
  patchPrescriptionGroup,
  patchPrescriptionReport,
...
type ReportWithAttachments = PrescriptionReport & {
...
function bestTitle(group: PrescriptionGroup, reports: ReportWithAttachments[]) {
  if (reports.length <= 1) return reports[0]?.title || group.title || "";
  return group.title || reports[0]?.title || "";
}

export default function PrescriptionGroupDetailsPage() {
  ...
  const [group, setGroup] = useState<PrescriptionGroup | null>(null);
  ...
  async function refresh() {
    ...
    setLoading(true);
    try {
      const details = await getPrescriptionGroupDetails(groupId);
  ...
  async function saveGroupTitle() {
    ...
    setError(null);
    try {
      const updated = await patchPrescriptionGroup({
        groupId,
  ...
  async function onDeleteGroup() {
    if (!groupId) return;
    const ok = window.confirm(t("prescriptionDeleteConfirm"));
    if (!ok) return;

    setError(null);
    setBusyGroup(true);
    try {
      await deletePrescriptionGroup(groupId);
  ...
  async function startShareCreate() {
    ...
    setShareResult(null);
    try {
      const res = await createPrescriptionShareLink({
        groupId,
        ttlSeconds: ttl,
      });
...
function ReportSheet({
  ...
  async function saveReport() {
    ...
    try {
      ...
      if (!initial) {
        const created = await createPrescriptionReport({
          groupId,
          ...patch,
        });
        reportId = created.id;
